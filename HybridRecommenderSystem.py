...
def users_with_the_same_movies(user_id, common_movie_thrshld):
    movies_watched = films_user_watched(user_movie_df, user_id)
    movies_watched_df = user_movie_df[movies_watched]
    user_movie_count = movies_watched_df.T.notnull().sum()
    user_movie_count = user_movie_count.reset_index()
    user_movie_count.columns = ["userId", "movie_count"]
    users_same_movies = user_movie_count[user_movie_count["movie_count"] > common_movie_thrshld]["userId"]
    return users_same_movies

def most_similar_user(df, user_id, common_movie_thrshld, corr_threshold):
    ...
    users_same_movies = users_with_the_same_movies(user_id, common_movie_thrshld)
